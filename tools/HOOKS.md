...
Git pre-commit に加えて、公式の UserPromptSubmit と Stop で自動整形します。[codex_hooks.py](codex_hooks.py) が依頼開始時の対象ファイルのハッシュを Git 管理ディレクトリ内に記録し、応答終了時に変化したファイルだけを整形します。生成ファイル・同梱パッケージ・Agent は既存 Git hook と同じ対象外です。Git が無視する未追跡ファイルも対象外です。

整形コマンドは Git hook と共通です。Codex 側では index を変更せず、ステージ済みファイル（部分ステージを含む）はスキップして警告します。コミット対象の整形は従来どおり pre-commit が行います。一時コピーで整形し、全対象の整形成功と編集状態の再確認後に作業ファイルだけを書き換えます。失敗時は理由と手動での再実行を案内し、応答の継続を強制しません。

開始時の記録がない場合は、無関係な差分を整形しないよう何も書き換えません。両イベントの定義を信頼してから新しい依頼を開始してください。ファイルの変化で対象を判断するため、依頼中に人や別タスクが変更したファイルと Codex の編集を厳密には区別できません。同じ作業ディレクトリでの同時編集は避けてください。中断された依頼の記録は Git 管理ディレクトリ内の arran-codex-format に残る場合があります。

Stop の単一ハンドラーで「整形 → 既存の完了通知」の順に実行します。品質チェックと生成物確認は Git 側のままです。整形後のコードについてテストを自動実行するものではありません。
...
- 公式形式は `<repo>/.codex/hooks.json` の `hooks.UserPromptSubmit` と `hooks.Stop`。コマンドには stdin で JSON が渡されます。
...
解除は `.codex/hooks.json` の Arran の UserPromptSubmit と Stop ハンドラーを削除します。このファイルが導入した内容だけならファイルを削除できます。他の hook が追加されている場合はファイル全体を削除しないでください。スクリプトだけを残しても通知は発火しません。
