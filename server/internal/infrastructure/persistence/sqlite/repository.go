...
// Migrate is kept as a repository-level facade for callers that explicitly
...
func (r *Repository) PutProfile(ctx context.Context, profile domain.Profile) error {
	...
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO profiles(
			id, monthly_income, current_balance, monthly_fixed_costs,
			monthly_free_budget, monthly_savings_goal, reduce_categories_json,
			allowed_categories_json, long_term_goal, advice_strictness, updated_at
		) VALUES(1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func (r *Repository) UpdateTransactionCategory(
	...
	if rememberMerchant {
		if _, err := r.db.ExecContext(ctx, `
			INSERT INTO merchant_rules(normalized_merchant, category, updated_at)
			VALUES(?, ?, ?)
...
func (r *Repository) MerchantRules(ctx context.Context) (map[string]string, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT normalized_merchant, category FROM merchant_rules
	`)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) CreateConsultation(
	...
	`, consultation.CreatedAt, consultation.UserMessage,
		...
		consultation.FollowUpQuestion)
	if err != nil {
		return domain.Consultation{}, err
	}
	consultation.ID, _ = result.LastInsertId()
...
func (r *Repository) AddMessage(
	...
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO consultation_messages(consultation_id, role, content, created_at)
		VALUES(?, ?, ?, ?)
...
func (r *Repository) SaveMemory(
	...
	if item.ID == 0 {
		...
		item.UpdatedAt = now
		result, err := r.db.ExecContext(ctx, `
			INSERT INTO memories(type, content, evidence, confidence, created_at, updated_at)
			VALUES(?, ?, ?, ?, ?, ?)
...
func (r *Repository) SaveReview(
	...
	result, err := r.db.ExecContext(ctx, `
		INSERT INTO monthly_reviews(month, created_at, summary, candidates_json)
		VALUES(?, ?, ?, ?)
	`, review.Month, review.CreatedAt, review.Summary, string(candidatesJSON))
	if err != nil {
		return domain.MonthlyReview{}, err
	}
	review.ID, _ = result.LastInsertId()
	return review, nil
}
...
func (r *Repository) LatestReview(ctx context.Context) (domain.MonthlyReview, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		...
		FROM monthly_reviews ORDER BY created_at DESC, id DESC LIMIT 1
