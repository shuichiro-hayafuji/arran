...
| ID | 対象・現状 | 当面の扱い・解消条件 |
| --- | --- | --- |
| EX-M01 | sessionは専用Dio・Secure Storage・ChangeNotifierを使用 | sessionの既存認証経路に限定許可。loginはpublicApiClientProviderと通常のRiverpod + MVIを使う。Session DomainのJSON変換はDTOへ分離済み。層を統一する際は認証・永続化・ユーザー切替の回帰テストを通す |
