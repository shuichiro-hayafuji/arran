...
| GAP-S01 | server内部は技術レイヤー単位の`application`・`handler`・Repository実装へ複数の業務機能が集まっており、ADR-0001の業務モジュール境界へ未移行 | API・業務ルール・所有者条件・現行の依存検査を維持し、機能変更に合わせて段階的に分割する。一括移行や空ディレクトリの先行作成はしない。移行時は依存チェッカーとテストを同じ変更で更新する |
